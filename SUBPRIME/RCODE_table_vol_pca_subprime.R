...
noms_modeles <- c("regress_CL_PCA_VOL", "regress_GC_PCA_VOL", "regress_SI_PCA_VOL", 
                  "regress_PA_PCA_VOL", "regress_HG_PCA_VOL", "regress_NG_PCA_VOL")
...
n1=nobs(modeles_importes$regress_CL_PCA_VOL)
n2=nobs(modeles_importes$regress_GC_PCA_VOL)
n3=nobs(modeles_importes$regress_SI_PCA_VOL)
n4=nobs(modeles_importes$regress_HG_PCA_VOL)
n5=nobs(modeles_importes$regress_PA_PCA_VOL)
n6=nobs(modeles_importes$regress_NG_PCA_VOL)
r1 <- round(summary(modeles_importes$regress_CL_PCA_VOL)$r.squared, 6)
r2 <- round(summary(modeles_importes$regress_GC_PCA_VOL)$r.squared, 6)
r3 <- round(summary(modeles_importes$regress_SI_PCA_VOL)$r.squared, 6)
r4 <- round(summary(modeles_importes$regress_HG_PCA_VOL)$r.squared, 6)
r5 <- round(summary(modeles_importes$regress_PA_PCA_VOL)$r.squared, 6)
r6 <- round(summary(modeles_importes$regress_NG_PCA_VOL)$r.squared, 6)


reg1=adjust_coefs_and_add_markers(modeles_importes$regress_CL_PCA_VOL)
reg2=adjust_coefs_and_add_markers(modeles_importes$regress_GC_PCA_VOL)
reg3=adjust_coefs_and_add_markers(modeles_importes$regress_SI_PCA_VOL)
reg4=adjust_coefs_and_add_markers(modeles_importes$regress_HG_PCA_VOL)
reg5=adjust_coefs_and_add_markers(modeles_importes$regress_PA_PCA_VOL)
reg6=adjust_coefs_and_add_markers(modeles_importes$regress_NG_PCA_VOL)
...
\\caption{Announcement and Financialization Effects on Futures Returns  (SUBPRIME sample (2007-2023))}
\\label{tab:return-fin-SUBPRIME}
\\centering
\\resizebox{\\linewidth}{!}{%
...
\\begin{tablenotes}
    \\singlespacing
    \\footnotesize
    This table presents estimates of eq. \\ref{eq:Model 1}, $R_{t}^{t+\\tau}=\\alpha+\\sum_{m=1}^{22} \\gamma_m S_{m,t}+ \\delta X_{t,i} + \\sum_{m=1}^{22} \\theta_m (S_{m,t} \\cdot X_t)+\\beta R_{t-\\tau}^{t}+\\epsilon_{t}$ using the method proposed by \\citet{ANDERSON2019price} and financialization variable $X_{t,2}=PCA_t$. The $\\gamma_m$ coefficients capture the instantaneous change in return when an announcement has just occurred and especially if that announcement was unanticipated. The coefficients $\\theta_m$ capture the instantaneous change in return when an announcement has just occurred in conjunction with the level of financialization.
\\end{tablenotes}
\\end{sidewaystable}
\\end{document}")
sink("Table_vol_PCA_SUBPRIME.tex")
