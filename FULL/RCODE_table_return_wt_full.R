...
noms_modeles <- c("regress_CL_WT_KUROV", "regress_GC_WT_KUROV", "regress_SI_WT_KUROV", 
                  "regress_PA_WT_KUROV", "regress_HG_WT_KUROV", "regress_NG_WT_KUROV")
...
n1=nobs(modeles_importes$regress_CL_WT_KUROV)
n2=nobs(modeles_importes$regress_GC_WT_KUROV)
n3=nobs(modeles_importes$regress_SI_WT_KUROV)
n4=nobs(modeles_importes$regress_HG_WT_KUROV)
n5=nobs(modeles_importes$regress_PA_WT_KUROV)
n6=nobs(modeles_importes$regress_NG_WT_KUROV)
r1 <- round(summary(modeles_importes$regress_CL_WT_KUROV)$r.squared, 6)
r2 <- round(summary(modeles_importes$regress_GC_WT_KUROV)$r.squared, 6)
r3 <- round(summary(modeles_importes$regress_SI_WT_KUROV)$r.squared, 6)
r4 <- round(summary(modeles_importes$regress_HG_WT_KUROV)$r.squared, 6)
r5 <- round(summary(modeles_importes$regress_PA_WT_KUROV)$r.squared, 6)
r6 <- round(summary(modeles_importes$regress_NG_WT_KUROV)$r.squared, 6)


reg1=adjust_coefs_and_add_markers(modeles_importes$regress_CL_WT_KUROV)
reg2=adjust_coefs_and_add_markers(modeles_importes$regress_GC_WT_KUROV)
reg3=adjust_coefs_and_add_markers(modeles_importes$regress_SI_WT_KUROV)
reg4=adjust_coefs_and_add_markers(modeles_importes$regress_HG_WT_KUROV)
reg5=adjust_coefs_and_add_markers(modeles_importes$regress_PA_WT_KUROV)
reg6=adjust_coefs_and_add_markers(modeles_importes$regress_NG_WT_KUROV)
...
\\begin{tablenotes}
    \\singlespacing
    \\footnotesize
    This table presents estimates of eq. \\ref{eq:Model 1}, $R_{t}^{t+\\tau}=\\alpha+\\sum_{m=1}^{22} \\gamma_m S_{m,t}+ \\delta X_{t,i} + \\sum_{m=1}^{22} \\theta_m (S_{m,t} \\cdot X_t)+\\beta R_{t-\\tau}^{t}+\\epsilon_{t}$ using the method proposed by \\citet{kurov2019price} and financialization variable $X_{t,2}=WT_t$. The $\\gamma_m$ coefficients capture the instantaneous change in return when an announcement has just occurred and especially if that announcement was unanticipated. The coefficients $\\theta_m$ capture the instantaneous change in return when an announcement has just occurred in conjunction with the level of financialization.
\\end{tablenotes}
\\end{sidewaystable}
\\end{document}")
sink("Table_return_WT_kurov_full.tex")
...
setwd("/Volumes/G-DRIVE ArmorATD/PHD/CH1/FULL")
noms_modeles <- c("regress_CL_WT_ANDERSON", "regress_GC_WT_ANDERSON", "regress_SI_WT_ANDERSON", 
                  "regress_PA_WT_ANDERSON", "regress_HG_WT_ANDERSON", "regress_NG_WT_ANDERSON")
...
n1=nobs(modeles_importes$regress_CL_WT_ANDERSON)
n2=nobs(modeles_importes$regress_GC_WT_ANDERSON)
n3=nobs(modeles_importes$regress_SI_WT_ANDERSON)
n4=nobs(modeles_importes$regress_HG_WT_ANDERSON)
n5=nobs(modeles_importes$regress_PA_WT_ANDERSON)
n6=nobs(modeles_importes$regress_NG_WT_ANDERSON)
r1 <- round(summary(modeles_importes$regress_CL_WT_ANDERSON)$r.squared, 6)
r2 <- round(summary(modeles_importes$regress_GC_WT_ANDERSON)$r.squared, 6)
r3 <- round(summary(modeles_importes$regress_SI_WT_ANDERSON)$r.squared, 6)
r4 <- round(summary(modeles_importes$regress_HG_WT_ANDERSON)$r.squared, 6)
r5 <- round(summary(modeles_importes$regress_PA_WT_ANDERSON)$r.squared, 6)
r6 <- round(summary(modeles_importes$regress_NG_WT_ANDERSON)$r.squared, 6)


reg1=adjust_coefs_and_add_markers(modeles_importes$regress_CL_WT_ANDERSON)
reg2=adjust_coefs_and_add_markers(modeles_importes$regress_GC_WT_ANDERSON)
reg3=adjust_coefs_and_add_markers(modeles_importes$regress_SI_WT_ANDERSON)
reg4=adjust_coefs_and_add_markers(modeles_importes$regress_HG_WT_ANDERSON)
reg5=adjust_coefs_and_add_markers(modeles_importes$regress_PA_WT_ANDERSON)
reg6=adjust_coefs_and_add_markers(modeles_importes$regress_NG_WT_ANDERSON)
l1=c(reg1[2],reg1[27],reg2[2],reg2[26],reg3[2],reg3[26],reg4[2],reg4[26],reg5[2],reg5[26],reg6[2],reg6[27])
l2 <- c(reg1[3], reg1[28], reg2[3], reg2[27], reg3[3], reg3[27], reg4[3], reg4[27], reg5[3], reg5[27], reg6[3], reg6[28])
l3 <- c(reg1[4], reg1[29], reg2[4], reg2[28], reg3[4], reg3[28], reg4[4], reg4[28], reg5[4], reg5[28], reg6[4], reg6[29])
l4 <- c(reg1[5], reg1[30], reg2[5], reg2[29], reg3[5], reg3[29], reg4[5], reg4[29], reg5[5], reg5[29], reg6[5], reg6[30])
...
\\end{sidewaystable}
\\end{document}")
sink("Table_return_WT_ANDERSON_full.tex")
cat(frame0,frame1,frame,tex1,tex2,tex3,tex4,tex5,tex6,tex7,tex8,tex9,tex10,tex11,tex12,tex13,tex14,tex16,tex17,tex18,tex19,tex20,tex21,tex22,frame2,tex23,tex24,frame3,frame4,frame5)
sink()
